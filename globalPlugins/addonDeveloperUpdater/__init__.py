...
class IssueTemplateEditorDialog(wx.Dialog):
    ...
    def _addLabeled(self, panel, sizer, label, control):
        labelControl = wx.StaticText(panel, label=label); labelControl.Wrap(380); sizer.Add(labelControl, 0, wx.ALIGN_CENTER_VERTICAL); sizer.Add(control, 1, wx.EXPAND)
    def _createFormPage(self):
        ...
        if self.document.kind == "markdown":
            outer.Add(wx.StaticText(panel, label=_("Instructions and questions shown to the person opening the issue:")), 0, wx.LEFT | wx.RIGHT | wx.TOP, 8)
            ...
            left.Add(wx.StaticText(panel, label=_("Questions and information blocks, in the order shown on GitHub:")), 0, wx.BOTTOM, 4); self.questionList = wx.ListBox(panel, style=wx.LB_SINGLE); left.Add(self.questionList, 1, wx.EXPAND)
            ...
            self.questionTypeValues = ("input", "textarea", "dropdown", "checkboxes", "markdown"); self.questionTypeNames = (_("Short answer, one line"), _("Long answer, multiple lines"), _("Drop-down choice list"), _("Checkbox list"), _("Information text, no answer"))
    ...
    def _enableQuestionFields(self):
        ...
        self.questionValueLabel.SetLabel(_("Information text shown in the issue form, written in Markdown:") if markdown else _("Answer filled in before the person starts typing:"))
